...
namespace RoyT.AStar
{
    ...
    {       
        ...
        public Grid(int dimX, int dimY, float defaultCost = 1.0f)
        {
            if (defaultCost < 1)
            {
                throw new ArgumentOutOfRangeException("Cost cannot be lower than one.");
        ...
        public void SetCellCost(Position position, float cost)
        {
            if (cost < 1)
            {
                throw new ArgumentOutOfRangeException("The cost of traversing a cell cannot be less than one");
        ...
        private int GetIndex(int x, int y)
        {
            if (x < 0 || x >= this.DimX)
            {
                throw new ArgumentOutOfRangeException("outside of the expected range");
        ...
        internal int GetIndexUnchecked(int x, int y) { return this.DimX * y + x; }
    }    
}
